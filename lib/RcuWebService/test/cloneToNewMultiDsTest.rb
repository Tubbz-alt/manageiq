...
#
# Create 2 NFS datastores and 4 clones, placing 2 clones on each datastore.
#
...
begin
	...
	vmFiles.each do |f|		
		files << RcuHash.new("Files") do |nf|
			nf.destDatastoreSpec = RcuHash.new("DestDatastoreSpec") do |dds|
				...
				dds.datastoreNames	= RcuArray.new do |dsna|
					dsna << "RichRcuTestTmp_1"
					dsna << "RichRcuTestTmp_2"
				end
				dds.goldVolume		= "RichRcuTestTmp"
				dds.numDatastores	= 2
				dds.protocol		= 'NFS'
				dds.sizeInMB		= 1024 * 3
	...
	clones = RcuHash.new("Clones") do |cl|
		cl.entry = RcuArray.new() do |ea|
			...
			ea << RcuHash.new("Entry") do |e|
				e.key	= "RcuCloneTestNewDs3"
				e.value	= ""
			end
			ea << RcuHash.new("Entry") do |e|
				e.key	= "RcuCloneTestNewDs4"
				e.value	= ""
			end
		end
	end
